...
def _plot_depend_cell_targeted(
    ax: plt.Axes,
    ...
    *,
    ...
    if target_feature not in x_df.columns or int_feature not in x_df.columns:
...
def make_targeted_depend_figure(target_feature: str, int_feature: str) -> None:
    ...
    for ri, scale in enumerate(SCALES):
        for ci, (scope, cz) in enumerate(COLUMN_SPECS):
            ...
            _plot_depend_cell_targeted(
                ax,
                shap_df, x_df, target_feature, int_feature,
    ...
    out_pdf = OUTPUT_DIR / f"shap_targeted_{safe_t}_vs_{safe_i}_5x3.pdf"
    out_png = OUTPUT_DIR / f"shap_targeted_{safe_t}_vs_{safe_i}_5x3.png"
    
    fig.savefig(out_pdf, dpi=300, bbox_inches="tight")
    fig.savefig(out_png, dpi=300, bbox_inches="tight")
    plt.close(fig)

    print(f"Saved targeted analysis:\n  -> {out_pdf}\n  -> {out_png}")
...
if __name__ == "__main__":
    ...
    print(f"[detail_analysis_pop] Generating targeted dependence figure:")
    print(f"  Target feature: {TARGET}")
    print(f"  Interaction feature: {INTERACTION}")
    
    make_targeted_depend_figure(TARGET, INTERACTION)
    print("Done!")
