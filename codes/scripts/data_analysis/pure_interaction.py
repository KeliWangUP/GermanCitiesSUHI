...
def _plot_pure_interaction_cell(
    ax: plt.Axes,
    int_vals: np.ndarray,
    x_sub: pd.DataFrame,
    target_feature: str,
    ...
    *,
    ...
    if target_feature not in x_sub.columns or int_feature not in x_sub.columns:
        ax.text(0.5, 0.5, f"Feature(s) missing\nnot selected in model", 
    ...
    int_idx = x_sub.columns.get_loc(int_feature)

    # 提取纯交互作用（由于 shap_interaction_values 是对称的，(i, j) 包含了一半的交互，完整交互作用为 (i, j) * 2）
    pure_interaction = int_vals[:, target_idx, int_idx].astype(float) * 2
    ...
    if show_ylabel:
        ax.set_ylabel("Pure Interaction Value", fontsize=9)
    else:
        ax.set_ylabel("")
...
def make_pure_interaction_figure(target_feature: str, int_feature: str) -> None:
    ...
    for ri, scale in enumerate(SCALES):
        for ci, (scope, cz) in enumerate(COLUMN_SPECS):
            ...
            _plot_pure_interaction_cell(
                ax,
                int_vals, x_sub, target_feature, int_feature,
    ...
    safe_i = int_feature.replace("_", "-")
    out_pdf = OUTPUT_DIR / f"shap_pure_interaction_{safe_t}_vs_{safe_i}_5x3.pdf"
    out_png = OUTPUT_DIR / f"shap_pure_interaction_{safe_t}_vs_{safe_i}_5x3.png"
    
    fig.savefig(out_pdf, dpi=300, bbox_inches="tight")
    fig.savefig(out_png, dpi=300, bbox_inches="tight")
    plt.close(fig)

    print(f"Saved pure interaction analysis:\n  -> {out_pdf}\n  -> {out_png}")

if __name__ == "__main__":
    TARGET = "ED_tree_cover"
    INTERACTION = "PLAND_tree_cover"
    
    print(f"[pure_interaction] Generating pure interaction figure:")
    print(f"  Target feature: {TARGET}")
    print(f"  Interaction feature: {INTERACTION}")
    
    make_pure_interaction_figure(TARGET, INTERACTION)
    print("Done!")
